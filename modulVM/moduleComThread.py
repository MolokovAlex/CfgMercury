...
class CommunicationCounterThread(QThread):
    ...
    # def read_InstantlyValue_full(self):
    #     """ считывание мгновенных значений и запись в БД 
    #     """
    #     for numCounter, itemCounter in enumerate(self.lstOfDic_Counters):
    #         net_adress_count = int(itemCounter['net_adress'])
    ...
    #         # if net_adress_count in [255,254,136,77]:
    #         # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    #             ml.logger.info(f"---------------поток: считывание МГНОВЕННЫХ значений со счетчика с NetAdress= {net_adress_count}-----------------")
    #             if mpm.test_canal_connection(net_adress_count):
    #                 if mpm.open_canal_connection_level1(net_adress_count):
    #                     # считывание мгновенных значений и запись в БД 
    #                     self.read_InstantlyValue(net_adress_count, itemCounter)
    ...
    # def read_ReadParam_full(self):
    #     """ считывание параметров счетчиков и запись в БД 
    #     """
    #     for numCounter, itemCounter in enumerate(self.lstOfDic_Counters):
    #         net_adress_count = int(itemCounter['net_adress'])
    #         if True:
    ...
    def getListCounterDB(self):
        ...
        lst_counterDB = []       
        with self.connectDB:
            self.cursor.execute("""SELECT id, schem, name_counter_full, net_adress, manuf_number, manuf_data, 
                                klass_react, klass_act, nom_u, ku, ki, koefA, comment FROM DBC ORDER BY net_adress ASC""")
    ...
    def editCounterDB(self, new_NameCounter:dict):
        ...
        try:
            with self.connectDB:
                    self.cursor.executemany("""UPDATE DBC SET schem=?, name_counter_full=?, net_adress=?, 
                                            manuf_number=?, manuf_data=?, klass_react=?, klass_act=?, nom_u=?, ku=?, ki=?, koefA=?, 
                                            comment=? WHERE id=?;""", (lst_newNameCounter,))
    ...
    #                             num_recordPP += 1
    #                             if num_recordPP >= 5: 
    #                                 ml.logger.debug("поток: вытащили 5 записей")
    #                                 return None # за каждую трехминутку вытаскиваем не более 5 записей
    #                         else:
    #                             ml.logger.debug("поток: предыдущюю запись профиля из счетчика уже есть в DBPP")
    #                     else:
    #                         ml.logger.debug("поток: не удалось считать предыдущюю запись профиля мощности счетчика")
