...
отключить 111 строку - запись в БД кофф KU kI

13052023_2
- исправил ось времени расчета профиля мощности - профиль должен начинаться в 0:30 и заканчиваться в 00:00 следующего дня
- исправил расчет Итого, переставил его выше по алгоритму до видоизменения табицы (30минут/1 час)

13052023
- исправил GUI меню профиля мощности - теерь нет кнопки выбора счетчика. Все дерево слева постоянно отображено

12052023
- исправил ошибку
bugs - при заполнение таблицы DBPP значением с datetime не происходит анализ что идит на update и что на insert - получаем 
в таблице DBPP много значений с одинаковыми  datetime

10052023
- закончил комммит 03052023

03052023
- переписал логику заполнения списков cfg.lst_id_checked_group и cfg.lst_id_checked_single_counter, чтобы при выборе группы с одиночным счетчиком
не происходил выбор просто одиночного счетчика

28042023
- вернул логику получения мгновенных значений - широковещательный запрос не сработывает почему-то. Вернемся на индивидуальные фиксации.

26042023
- отключил поток и математику LOSTDATAPP
- изменил логику получения мгновенных значений - сначала выдается широковещательный запрос на фиксацию датывремени, а потом так жеиндивидуально сибираются мгновенные значения
- изменил логику вытягивания профилей мощности: таблица LOSTDATAPP хранит пару id_counter/адрес последней считанной записи профиля. Начинаем с адреса последней записи профиля на текущий момент и с каждым вычитыванием адрес уменьшается до 0х0010. Потом опять на адрес последней записи профиля на текущий момент.

25042023
- включил поток LOSTDATAPP, но он на большом количестве н работает из-за: вычисляем адрес предполагаемого штампа датывремени, запрашиваем от счетчика,
но в ответ приходит совершенно другой датештамп. По документации сказано - смотреть флаги. Установлен флаг переполнения массива профилей. И что делать?
